"""
LoRA (Low-Rank Adaptation) variants for FlashOCR.
...
All variants are adapted for convolutional OCR backbones.
"""
...
def apply_lora(
    ...
    """Apply LoRA adapters to target modules of a FlashOCR model.

    Freezes all pretrained weights and only makes LoRA A/B matrices trainable.

    Args:
        model: FlashOCR instance.
    ...
    if target_modules is None:
        target_modules = ["backbone", "encoder"]
    ...
    # Unfreeze LoRA parameters + decoder (always trainable for fine-tuning)
