"""Profiler — layer-wise latency and memory analysis for FlashOCR models."""
...
class Profiler:
    """Profile a FlashOCR PyTorch model layer-by-layer.

    Hooks into every leaf ``nn.Module`` to record per-layer forward time
    and (optionally) memory consumption.

    Parameters
    ----------
    model_path : str | Path
        Path to a FlashOCR ``.pth`` checkpoint.
    device : str
        ``"cuda"`` or ``"cpu"``.
    ...
    def parameter_count(self) -> Dict[str, int]:
        ...
        -------
        dict
            ``{"total", "trainable", "frozen", "backbone", "encoder", "decoder"}``
        """
        model = self._load_model()
        ...
        for component in ("backbone", "encoder", "decoder"):
            sub = getattr(model, component, None)
            if sub is not None:
                component_counts[component] = sum(p.numel() for p in sub.parameters())
            else:
